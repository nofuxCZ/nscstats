...
export default function NationPage() {
  ...
  const qrDisplay = p.qr != null ? String(p.qr) + "%" : "\u2014";
  const qrSub = String(p.sfQ || 0) + "/" + String(p.sf || 0);
  const qrColor = p.qr != null ? (p.qr >= 70 ? "var(--green)" : p.qr >= 50 ? "var(--gold)" : "var(--red)") : "var(--text-30)";
    ...
    <div style={{ maxWidth: 1100, margin: "0 auto", padding: "24px" }}>
      ...
      <div className="fi" key={sel} style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 16 }}>
          <div>
            ...
            <div style={{ fontSize: 13, color: "var(--text-35)", marginTop: 6 }}>
              {"Active since #" + String(p.fe) + " \u00B7 Last seen in #" + String(p.le) + " \u00B7 " + String(p.te) + " editions total"}
            </div>
          </div>
          {p.w > 0 && (
            <div style={{ display: "flex", gap: 2 }}>
              {Array.from({ length: Math.min(p.w, 12) }).map(function(_, i) {
                return <span key={i} style={{ fontSize: 20 }} title={"Win: Edition " + String((p.we || [])[i] || "?")}>🏆</span>;
              })}
            </div>
          )}
        </div>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 2, marginTop: 20, background: "var(--card-dark)", borderRadius: 12, padding: "6px 2px" }}>
          ...
          <St v={qrDisplay} l="QF Rate" c={qrColor} sub={qrSub} />
      ...
      {h.length > 0 && (
        <div style={{ marginTop: 24 }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: "var(--text-60)", marginBottom: 10 }}>
            {"Edition History (" + String(h.length) + " editions)"}
          </div>
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: "0 2px", minWidth: 700 }}>
              <thead>
                <tr>
                  {["Ed.", "SF", "SF Pl.", "SF Pts", "GF Pl.", "GF Pts", "Artist", "Song"].map(function(hd, i) {
                    return <th key={hd} style={{
              ...
              <tbody>
                {hs.map(function(r) {
                  ...
                  var bg = isW ? "rgba(255,215,0,0.05)" : isM ? "var(--text-02)" :
                  ...
                  return (
                    <tr key={e}>
                      ...
                      <td style={{ padding: "8px 10px", textAlign: "center", background: bg }}>
                        {sfS ? <span style={{ fontSize: 11, fontWeight: 700, padding: "2px 6px", borderRadius: 3, background: sfS === "SF1" ? "var(--blue-glow-15)" : "var(--purple-glow-15)", color: sfS === "SF1" ? "var(--blue)" : "var(--purple)" }}>{String(sfS)}</span>
                      ...
                      <td style={{ padding: "8px 10px", textAlign: "center", background: bg }}>
                        {mc[gfP] ? <span style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: 26, height: 26, borderRadius: "50%", fontWeight: 700, fontSize: 12, background: mc[gfP], color: "var(--btn-body)" }}>{String(gfP)}</span>
                          : isPq ? <span style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: 26, height: 26, borderRadius: "50%", fontWeight: 700, fontSize: 12, background: "var(--blue-glow-20)", border: "1.5px solid var(--blue-glow-50)", color: "var(--blue)" }}>{String(gfP)}</span>
                          ...
                          : gfP ? <span style={{ color: "var(--text-45)" }}>{String(gfP)}</span>
                          : <span style={{ color: "var(--text-10)" }}>{"\u2014"}</span>}
                      </td>
