  ...
  return (
    <div style={{ maxWidth: 1100, margin: "0 auto", padding: "24px" }}>
      ...
      <div className="fi" key={sel} style={{
        marginTop: 24,
        background: "var(--nation-card-bg)", border: "1px solid var(--nation-card-border)",
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 16 }}>
          <div>
            ...
            <div style={{ fontSize: 13, color: "var(--text-35)", marginTop: 6 }}>
              Active since #{p.fe} · Last in #{p.le} · {p.te} editions
            </div>
          </div>
          {p.w > 0 && (
            <div style={{ display: "flex", gap: 2 }}>
              {Array.from({ length: Math.min(p.w, 12) }).map((_, i) => (
                <span key={i} style={{ fontSize: 20 }} title={`Win #${i + 1}: Edition ${p.we[i]}`}>🏆</span>
              ))}
            </div>
          )}
        </div>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 2, marginTop: 20, background: "var(--card-dark)", borderRadius: 12, padding: "6px 2px" }}>
          ...
          <St v={p.qr !== null ? `${p.qr}%` : "—"} l="QF Rate"
            c={p.qr >= 70 ? "var(--green)" : p.qr >= 50 ? "var(--gold)" : "var(--red)"} />
                  ...
                  return (
                    <tr key={e}>
                      ...
                      <td style={{ padding: "8px 10px", textAlign: "center", background: bg }}>
                        {sfS ? (
                          <span style={{ fontSize: 11, fontWeight: 700, padding: "2px 6px", borderRadius: 3, background: sfS === "SF1" ? "var(--blue-glow-15)" : "var(--purple-glow-15)", color: sfS === "SF1" ? "var(--blue)" : "var(--purple)" }}>{sfS}</span>
                        ) : gfP ? (
                          <span style={{ fontSize: 10, color: "var(--gold-text-40)", fontWeight: 600 }}>PQ</span>
                        ) : (
                      ...
                      <td style={{ padding: "8px 10px", textAlign: "center", background: bg }}>
                        {mc[gfP] ? (
                          <span style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: 26, height: 26, borderRadius: "50%", fontWeight: 700, fontSize: 12, background: mc[gfP], color: "var(--btn-body)" }}>{gfP}</span>
                        ) : isPq ? (
                          <span style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: 26, height: 26, borderRadius: "50%", fontWeight: 700, fontSize: 12, background: "var(--blue-glow-20)", border: "1.5px solid var(--blue-glow-50)", color: "var(--blue)" }}>{gfP}</span>
                        ) : dnq ? (
                          <span style={{ fontSize: 11, fontWeight: 700, color: "var(--red-text)", background: "var(--red-light)", padding: "2px 8px", borderRadius: 4 }}>DNQ</span>
                        ) : gfP ? (
                          <span style={{ color: "var(--text-45)" }}>{gfP}</span>
                        ) : (
                          <span style={{ color: "var(--text-10)" }}>—</span>
                        )}
                      </td>
